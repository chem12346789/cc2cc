...
for cycle in {1..1}; do
	...
	# 	~/anaconda3/envs/pyscf/bin/python test.py -dl ${dl_args} --basis ${basis_args} --extend_atom 0-1 --extend_xyz 0 \
	# 		--precision float64 ${load_args} --density_restriction 1 || exit 1
	# else
	# 	~/anaconda3/envs/pyscf/bin/python gen_data.py -dl ${dl_args} --basis ${basis_args} --extend_atom 0-1 --extend_xyz 0 || exit 1
	# fi
	~/anaconda3/envs/pyscf/bin/python train.py -dl ${dl_args} --basis ${basis_args} --extend_atom 0-1 --extend_xyz 0 \
		--eval_step 10 --epoch 200100 --with_eval False --precision float32 --save_dir cycle${cycle} \
