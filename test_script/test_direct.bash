...
export load_args="atom-1-3677263"
...
if [ -z "$n_rad_args" ]; then
    echo "--load ${load_args}"
    ~/anaconda3/envs/pyscf/bin/python test.py ${mol_args} --precision float64 --load ${load_args} --load_epoch -24000 --dataset g2 ${MODEL} --if_continue 0
else
    echo "Random initialization"
    ~/anaconda3/envs/pyscf/bin/python test.py ${mol_args} --precision float64 --dataset g2 ${MODEL} --if_continue 0
fi
