...
class ModelDict:
    ...
    def get_nev(
        ...
        rho_norm_matrix = grids.gen_grids_matrix(ks.mol, dms, reset=True)
        # mask_cube = np.zeros((1, 4, 3, 3, 3))
        # mask_cube[:, :, 1, 1, 1] = 1.0
        # print(rho_cube[:10])
        # rho_cube = rho_cube * mask_cube
        # print(rho_cube[:10])
        input_mat = torch.tensor(rho_norm_matrix, dtype=self.dtype, device=self.device)
        ...
        energy_den = grids.matrix_to_vector(
