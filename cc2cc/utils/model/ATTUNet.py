...
class unet_down(nn.Module):
    def __init__(self, in_size, out_size):
        ...
        self.conv_block = conv_block(ch_in=in_size, ch_out=out_size)
...
class unet_up(nn.Module):
    def __init__(self, in_size, out_size):
        super().__init__()
        self.up = up_conv(ch_in=in_size, ch_out=out_size)
        self.conv_block = conv_block(ch_in=in_size, ch_out=out_size)

    def forward(self, x1, x2):
        x2 = self.up(x2)
        x2 = pad_cat(x1, x2, dim=1)
...
#         self.Conv1 = conv_block(ch_in=img_ch, ch_out=self.hidden_ch)

#         self.down_layers = nn.ModuleList(
#             [
#                 unet_down(
#                     self.hidden_ch * 2 ** (i),
#                     self.hidden_ch * 2 ** (i + 1),
#                 )
...
#             self.hidden_ch, output_ch, kernel_size=1, stride=1, padding=0
#         )
