...
def make_intermediates(mycc, t1, t2, eris):
    ...
    rw = r6(p6(w)) / d3
    imds.l1a_t = (
        numpy.einsum("ijkabc,jbkc->ia", rw.conj(), numpy.asarray(eris.ovov))
        / eia
        * 0.25
    )
    wvd = r6(p6(w * 2 + v)) / d3
    ...
    imds.l2aa_t = l2_t.conj() / lib.direct_sum("ia+jb->ijab", eia, eia) * 0.5
    ...
    l2_t = l2_t + l2_t.transpose(1, 0, 3, 2)
    l2_t += numpy.einsum("ijkabc,ai->jkbc", rw, fVO)
    imds.l2bb_t = l2_t.conj() / lib.direct_sum("ia+jb->ijab", eIA, eIA) * 0.5
    ...
    rw = r4(w) / d3
    imds.l1a_t += (
        numpy.einsum("ijkabc,kcia->jb", rw.conj(), numpy.asarray(eris.ovOV)) / eia * 0.5
    )
    imds.l1b_t += (
        numpy.einsum("ijkabc,jbkc->ia", rw.conj(), numpy.asarray(eris.ovov))
        / eIA
    ...
    l2_t += numpy.einsum(
        "ijkabc,kcea->jibe", wvd, numpy.asarray(eris.get_ovVV()).conj()
    )
    ...
    imds.l2ab_t = l2_t.conj() / lib.direct_sum("ia+jb->ijab", eia, eIA) * 0.5
    ...
    imds.l1b_t += (
        numpy.einsum("ijkabc,iakc->jb", rw.conj(), numpy.asarray(eris.ovOV)) / eIA * 0.5
    )
    wvd = r4(w * 2 + v) / d3
    l2_t = numpy.einsum("ijkabc,iaec->jkbe", wvd, numpy.asarray(eris.get_ovVV()).conj())
    l2_t -= numpy.einsum("ijkabc,iakm->jmbc", wvd, numpy.asarray(eris.ovOO).conj())
    ...
    l2_t = numpy.einsum("ijkabc,kceb->ijae", wvd, numpy.asarray(eris.get_OVVV()).conj())
    l2_t += numpy.einsum(
        "ijkabc,kcea->ijeb", wvd, numpy.asarray(eris.get_OVvv()).conj()
    )
