...
def u_gamma2_intermediates(mycc, t1, t2, l1, l2, eris=None, compress_vvvv=False):
    ...
    for b0, b1 in lib.prange(0, nvira, blksize):
        for c0, c1 in lib.prange(0, nvira, blksize):
            ...
            dovov[:, b0:b1, :, c0:c1] += numpy.einsum("ia,ijkabc->jbkc", t1a, rw) * 0.25
            ...
            dooov -= (
                numpy.einsum("mkbc,ijkabc->jmia", t2aa[:, :, b0:b1, c0:c1], wvd) * 0.125
            )
            dovvv[:, :, :, b0:b1] += (
                numpy.einsum("kjcf,ijkabc->iafb", t2aa[:, :, c0:c1, :], wvd) * 0.125
            )
        time2 = log.timer_debug1(
    ...
    for b0, b1 in lib.prange(0, nvirb, blksize):
        for c0, c1 in lib.prange(0, nvirb, blksize):
            ...
            dOVOV[:, b0:b1, :, c0:c1] += numpy.einsum("ia,ijkabc->jbkc", t1b, rw) * 0.25
            dOOOV -= (
                numpy.einsum("mkbc,ijkabc->jmia", t2bb[:, :, b0:b1, c0:c1], wvd) * 0.125
            )
            dOVVV[:, :, :, b0:b1] += (
                numpy.einsum("kjcf,ijkabc->iafb", t2bb[:, :, c0:c1, :], wvd) * 0.125
            )
        time2 = log.timer_debug1(
    ...
    for a0, a1 in lib.prange(0, nvirb, blksize):
        for b0, b1 in lib.prange(0, nvira, blksize):
            for c0, c1 in lib.prange(0, nvira, blksize):
                ...
                dovvv[:, c0:c1, :, b0:b1] += (
                    numpy.einsum("jiea,ijkabc->kceb", t2ab[:, :, :, a0:a1], wvd) * 0.25
                )
                dovVV[:, c0:c1, :, a0:a1] += (
                    numpy.einsum("jibe,ijkabc->kcea", t2ab[:, :, b0:b1, :], wvd) * 0.25
                )
                dOVvv[:, a0:a1, :, c0:c1] += (
                    numpy.einsum("jkbe,ijkabc->iaec", t2aa[:, :, b0:b1, :], wvd) * 0.125
                )
                dooov[:, :, :, c0:c1] -= (
                    numpy.einsum("miba,ijkabc->jmkc", t2ab[:, :, b0:b1, a0:a1], wvd)
                    * 0.25
                )
                dOOov[:, :, :, c0:c1] -= (
                    numpy.einsum("jmba,ijkabc->imkc", t2ab[:, :, b0:b1, a0:a1], wvd)
                    * 0.25
                )
                dooOV[:, :, :, a0:a1] -= (
                    numpy.einsum("jmbc,ijkabc->kmia", t2aa[:, :, b0:b1, c0:c1], wvd)
                    * 0.125
                )
                dovov[:, b0:b1, :, c0:c1] += (
                    numpy.einsum("ia,ijkabc->jbkc", t1b[:, a0:a1], rw) * 0.25
                )
                # dOVov[:, a0:a1, :, c0:c1] += numpy.einsum('jb,ijkabc->iakc', t1a, rw) * .25
                dovOV[:, c0:c1, :, a0:a1] += (
                    numpy.einsum("jb,ijkabc->kcia", t1a[:, b0:b1], rw) * 0.25
                )
        time2 = log.timer_debug1(
    ...
    for a0, a1 in lib.prange(0, nvira, blksize):
        for b0, b1 in lib.prange(0, nvirb, blksize):
            for c0, c1 in lib.prange(0, nvirb, blksize):
                ...
                wvd = (w_blk * 2 + v_blk) / d3
                dOVVV[:, c0:c1, :, b0:b1] += (
                    numpy.einsum("ijae,ijkabc->kceb", t2ab[:, :, a0:a1, :], wvd) * 0.25
                )
                dOVvv[:, c0:c1, :, a0:a1] += (
                    numpy.einsum("ijeb,ijkabc->kcea", t2ab[:, :, :, b0:b1], wvd) * 0.25
                )
                dovVV[:, a0:a1, :, c0:c1] += (
                    numpy.einsum("jkbe,ijkabc->iaec", t2bb[:, :, b0:b1, :], wvd) * 0.125
                )
                dOOOV[:, :, :, c0:c1] -= (
                    numpy.einsum("imab,ijkabc->jmkc", t2ab[:, :, a0:a1, b0:b1], wvd)
                    * 0.25
                )
                dooOV[:, :, :, c0:c1] -= (
                    numpy.einsum("mjab,ijkabc->imkc", t2ab[:, :, a0:a1, b0:b1], wvd)
                    * 0.25
                )
                dOOov[:, :, :, a0:a1] -= (
                    numpy.einsum("jmbc,ijkabc->kmia", t2bb[:, :, b0:b1, c0:c1], wvd)
                    * 0.125
                )
                dOVOV[:, b0:b1, :, c0:c1] += (
                    numpy.einsum("ia,ijkabc->jbkc", t1a[:, a0:a1], rw) * 0.25
                )
                dovOV[:, a0:a1, :, c0:c1] += (
                    numpy.einsum("jb,ijkabc->iakc", t1b[:, b0:b1], rw) * 0.25
                )
