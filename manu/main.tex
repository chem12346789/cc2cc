...
%
The input of the neural network is a set of the energy density ${\bm{e}_{\rm{cube}}(\brm{r}_i)}$ with the size of $(N, 27, 4)$, and the output is the energy density ${E(\brm{r}_i)}$ with the size of $(N, 1)$.
% 
...
\begin{align}
	...
	\bm{e}_{\rm{cube}}(\brm{r}_{i}) =
	 & \big\{ \bm{e}(\mathbf{r}'_{i}) \text{ for } \mathbf{r}'_{i} \in \mathbf{r}_{i;cube} \big\}.    \\
	\bm{e}(\brm{r}) =
...
\begin{align}
	\label{eq:loss}
	L_{e} =
	 & \frac{1}{N_{\rm{atom}}} \Big| \int \d \brm{r}_{i} \big(E_{\mathrm{target}}(\brm{r}_{i}) - E_{\mathrm{predict}}(\brm{r}_{i})\big) \Big| \nl
	 & \quad + \int \d \brm{r}_{i} \Big| E_{\mathrm{target}}(\brm{r}_{i}) - E_{\mathrm{predict}}(\brm{r}_{i}) \Big|.
\end{align}
