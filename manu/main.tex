...
\end{align}
The $\tilde{e}(\brm{r})$ is the energy density calculated by the LDA \cite{Koh65A1133}, VWN3 \cite{Vos80A789}, B88 \cite{Bee88A785}, and LYP \cite{Lee88A785} functionals.
% 
...
%
We use 551 molecules which consistent of less than 3 heavy atoms as the training set and 115 molecules as the validation set.
% 
In this article, we use the CCSD(T)/Def2-QZVP and CCSD(T)/Ma-Def2-QZVP \cite{weigendBalancedBasisSets2005,zhengMinimallyAugmentedKarlsruhe2011-02} as the target energy of the molecules.
...
\begin{align}
	\label{eq:loss}
	L_{e} =
	 & \lambda_{\rm{RE}} \Big( \sqrt{N_{\rm{atom}}} \int \d \brm{r}_{i} \big(\bar{e}(\brm{r}_{i}) - e(\brm{r}_{i})\big) \Big)^2 \nl
	 & + \lambda_{\rm{ABS}} \Big( \sqrt{N_{\rm{atom}}} \int \d \brm{r}_{i} \Big| \bar{e}(\brm{r}_{i}) - e(\brm{r}_{i}) \Big| \Big)^2 \nl
	 & + \lambda_{\rm{AE}} \Big( \sqrt{N_{\rm{atom}}} \bar{E}^{\rm{AE}}(\brm{r}_{i}) - E^{\rm{AE}}(\brm{r}_{i}) \Big)^2 .
\end{align}
